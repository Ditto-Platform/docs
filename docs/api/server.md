---
title: 服务端 API 参考
description: Ditto WebOS 服务端 HTTP + WebSocket 接口完整参考：基础信息、认证、应用安装与管理、注册表、市场、认证、管理、代理、VFS、应用 Cell 后端、WebSocket、错误响应与资源配额限流。
outline: [2, 3]
---

# 服务端 API 参考

Ditto WebOS 服务端由 Hono 实现，对外暴露 HTTP 与 WebSocket 两类接口，覆盖应用安装、注册表、市场、用户认证、Cell 后端代理、虚拟文件系统、网络代理与系统管理。本页面是这些路由的完整参考手册。
...
- [认证机制](#认证机制)
- [根路径与健康检查](#根路径与健康检查)
- [应用安装与管理（/api/apps）](#应用安装与管理-api-apps)
- [应用注册表（/api/app-registry）](#应用注册表-api-app-registry)
- [应用市场（/api/market）](#应用市场-api-market)
- [认证（/api/auth）](#认证-api-auth)
- [管理（/api/admin）](#管理-api-admin)
- [网络代理（/api/proxy）](#网络代理-api-proxy)
- [虚拟文件系统（/api/vfs）](#虚拟文件系统-api-vfs)
- [应用 Cell 后端（/api/cell）](#应用-cell-后端-api-cell)
- [WebSocket 端点（/ws）](#websocket-端点-ws)
- [错误响应格式](#错误响应格式)
- [资源配额与限流](#资源配额与限流)
- [示例](#示例)
...
::: tip 端口与应用目录
应用安装目录由 `APPS_DIR` 控制，默认 `data/apps`；市场测试应用目录由 `TEST_APPS_DIR` 控制。生产部署可通过环境变量覆盖，详见[配置项](/deployment/configuration)。
:::
...
::: warning Token 非加密签名
当前 Token 仅是 base64url 编码的 JSON，并非加密签名。生产环境如需更高强度，请在前置网关层叠加 JWT 或 mTLS 校验。
:::
...
- `.ditx`：扩展应用包
- `.ditc`：组件包
- `.ditz`：加密包（需配合 `X-Decrypt-Password`）
:::
...
::: tip 数据源优先级
本地 `MARKET_DATA_DIR` → `TEST_APPS_DIR` → 内置 demo 数据 → GitHub 仓库 `Nevino2233/Ditto_Market`（可用时）。市场数据缓存 5 分钟，评论缓存 2 分钟。
:::
...
::: danger 生产环境务必加鉴权
当前代码未对 admin 路由强制鉴权，任何能访问服务端的人都可以查询与操控 Cell。生产部署**必须**在前置网关层叠加权限校验（如 nginx basic auth、Cloudflare Access、零信任网关等）。
:::
...
::: warning 默认规则
屏蔽 `localhost` / `127.0.0.1` / `0.0.0.0` / `::1`；最大响应体 50 MB；超时 30 s；启用缓存（按 `Cache-Control: max-age` 失效，最多 500 条）。响应头会附加 `X-Proxy-Status`、`X-Proxy-StatusText`、`X-Proxy-Cached` 等。
:::
...
::: warning 内存实现
当前 VFS 完全基于内存 Map，服务端重启后数据全部丢失。仅适合存放临时文件、Cell 中转数据，请勿作为持久存储使用。
:::
...
::: tip 前后端对称
前端 Cell 通过 [`CellBridge`](./client#cellbridge-api) 与本节后端 Cell 通信：`notifyStart` → `notifyHibernate` → `notifyWake` → `notifyStop`，对应表中四个生命周期端点。
:::
...
::: warning 常见状态码
`400`（参数/包无效）、`401`（未认证）、`403`（代理域名禁止）、`404`（资源不存在）、`409`（评论冲突）、`429`（限流/超配额）、`500`（服务端异常）、`502`（上游/下载失败）、`503`（Cell 不可用或流量整形器未启用）。
:::
...
```bash
curl http://localhost:3001/api/apps/list
```

### 浏览市场应用（按评分排序）

```bash
curl "http://localhost:3001/api/market/apps?sort=rating&category=productivity"
```

### 注册外部应用

```bash
curl -X POST http://localhost:3001/api/app-registry/register \
  -H "Content-Type: application/json" \
  -d '{
    "id": "com.example.app",
    "name": "Example",
    "version": "1.0.0",
    "entryUrl": "https://example.com/index.html",
    "iconUrl": "",
    "description": ""
  }'
```
...
curl -X PUT http://localhost:3001/api/auth/profile \
  ...
  -H "Content-Type: application/json" \
  -d '{"username":"alice_new","avatar":"https://example.com/a.png"}'
```

### 写入 VFS 文件

```bash
curl -X PUT http://localhost:3001/api/vfs/write/data/notes/hello.txt \
  -H "Content-Type: application/octet-stream" \
  --data-binary "Hello Ditto"
```

### 调用网络代理

```bash
curl -X POST http://localhost:3001/api/proxy/fetch \
  -H "X-Proxy-Target: https://api.github.com/repos/Nevino2333/Ditto" \
...
- [部署运维 — 生产部署](/deployment/production)
