---
title: 前端 API 参考
description: Ditto WebOS 前端 TypeScript API 完整参考：Kernel、ServiceRegistry、AppCellManager、ClientCell、CellBridge、PermissionManager、IPCBus、EventEmitter 及内核事件一览表。
outline: [2, 3]
---

# 前端 API 参考

Ditto WebOS 前端 API 由 `@ditto/core` 与 `@ditto/services` 暴露，覆盖内核控制、服务编排、Cell 生命周期、跨沙盒 IPC、权限管理与事件总线。本页面是这些 API 的严谨参考手册，所有签名与源码保持一致。
...
::: tip 取代全局单例
`getKernel()` 已废弃。所有需要内核的代码请通过依赖注入或显式参数传递 `kernel` 实例，避免隐式全局状态。
:::
...
::: warning 终态不可逆
进入 `destroyed` 后再次调用 `init()` 行为未定义。如需重启内核，请创建新的 `DittoKernel` 实例。
:::
...
::: danger 不可恢复
`unload` 之后的 ClientCell 不应再使用。如需重新启动，请通过 `AppCellManager.startCell` 创建新实例。
:::
...
::: warning 非阻塞语义
`sendToServer` 不会等待 WS 处于 open 状态。若在连接建立前调用，消息将被丢弃，仅在控制台输出 warn。需要保证送达时请使用 `request` + `respond` 模式。
:::
...
::: tip 生产模式接入交互式 UI
生产模式下 `request` 默认拒绝。Shell 通常会注入交互式 prompt 回调，弹出确认对话框征求用户同意，详见[权限系统概念](/concepts/permission)。
:::
...
`(string & {})` 是 TypeScript 的常见技巧，允许任意字符串但保留自动补全。
...
::: tip 中间件洋葱模型
中间件按注册顺序包裹执行。若任意中间件未调用 `next(msg)`，后续中间件与最终 handler 都不会触发。可用于鉴权、限流、日志埋点。
:::
...
::: danger 严禁使用 * 通配
IPCBus v2 不再允许 `origin: '*'`，所有 IFrameSandbox 必须显式设置 origin，避免恶意页面冒充 Cell 收发消息。
:::
...
::: tip 错误隔离
EventEmitter 会捕获单个 handler 的同步异常并派发 `error:handler` 事件，避免一个监听器抛错影响其他监听器。但异步错误（Promise rejection）不会被捕获，请 handler 内部自行 `try/catch`。
:::
