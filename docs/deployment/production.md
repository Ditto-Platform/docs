---
title: 生产部署
description: Ditto WebOS 完整生产部署指南——开发环境、Docker Compose、手动部署、Dockerfile、nginx、PWA、环境变量、性能优化与 Chrome 80 兼容。
---
...
::: warning 注意
turbo 会同时启动多个 dev server，终端输出会混合。开发时建议分别启动 server 与 shell 以便调试。
:::
...
::: tip 镜像优化建议
如需进一步缩小镜像，可使用多阶段构建：第一阶段构建前端，第二阶段仅复制 `apps/shell/dist` 到 nginx 镜像。Server 镜像也可基于 `oven/bun:1-slim` 减少体积。
:::
...
::: warning WebSocket 超时
默认 nginx `proxy_read_timeout` 为 60s，会导致空闲 WebSocket 连接被断开。务必设置为 `86400`（24 小时）或更大值。
:::
...
::: tip 进程管理选型
- **systemd**：Linux 原生，无额外依赖，推荐生产环境使用
- **PM2**：跨平台，自带日志轮转与负载均衡，适合开发 / 多环境切换
- **Docker**：最佳隔离与可移植性，配合 Compose 一键启动
:::
...
::: warning 目录命名规则
应用 ID 中的 `.` 字符会被替换为 `_`。例如 `com.ditto.calc` 对应目录 `com_ditto_calc`。手动放置应用包时请遵守该规则。
:::
...
::: warning API 离线限制
PWA 不缓存 `/api/*` 请求，离线状态下应用前端可加载但后端 Cell 无法访问。需要离线后端能力的场景，请考虑将 Cell 后端打包为前端 Web Worker 模式。
:::
...
| `DITTO_CORS_ORIGIN` | `*` | CORS 白名单（逗号分隔，生产环境收紧） |
...
::: tip 隔离性
应用专属环境变量只对该应用后端 Cell 可见，不会污染其他应用或宿主进程。这是 Cell 沙盒机制的一部分。
:::
...
::: tip 优化优先级
1. 先用 [监控运维](./monitoring) 查看实际瓶颈
2. Cell 数量多但空闲多 → 调小 `hibernateAfterMs`
3. 单个应用内存高 → 调小 `DEFAULT_CELL_QUOTA.memoryMB`
4. 用户量大 → 启用 `replica: 'shared'` 共享只读 Cell
:::
...
::: tip 内网部署完整清单
- ✅ 下载 `regenerator-runtime` 到 `public/`
- ✅ 修改 `vite.config.ts` 指向本地路径
- ✅ 配置 nginx 缓存策略
- ✅ 验证 Chrome 80 加载 legacy bundle 正常
:::

## 相关文档

- [部署概览](./index) — 部署总览与导航
- [教育场景部署](./education) — 学校机房、树莓派、Kiosk 模式
- [配置项](./configuration) — `DittoConfig` 完整字段与环境变量
- [监控运维](./monitoring) — 健康检查、Cell 状态、系统指标
- [核心概念 Cell](../concepts/cell) — Cell 沙盒与 IPC 架构
